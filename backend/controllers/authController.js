...
export const sendUserOTP = async (req, res) => {
  ...
  setTimeout(() => otpStore.delete(phone), 300000); // OTP expires in 30 sec
...
// ✅ Verify OTP and Register New User with ₹20 balance
// ✅ Verify OTP and Register New User with ₹20 balance + referral reward
export const verifyUserOTP = async (req, res) => {
  const { phone, otp, password, referralCode } = req.body; // referralCode from frontend
...
// ✅ User Login (includes Admin check)
...
export const loginAdmin = (req, res) => {
  ...
  console.log('Expected phone:', process.env.ADMIN_PHONE);
...
// Send OTP for password reset (same as sendUserOTP but phone must exist)
...
export const verifyResetPasswordOTP = async (req, res) => {
  ...
  // User must exist (checked during send OTP)
